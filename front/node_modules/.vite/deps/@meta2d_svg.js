...
// node_modules/@meta2d/svg/src/svg.js
...
function parseSvg(svg) {
  ...
  anchorsArr = [];
  svgs.forEach((svg2) => {
    const selfProperty = svg2[selfName];
    allRect = transformContainerRect(selfProperty);
    const isWidthLitter = allRect.width < allRect.height;
    if (!isWidthLitter) {
      if (!selfProperty.width) {
        selfProperty.width = 40;
      }
      shapeScale = selfProperty.width / allRect.width;
      !selfProperty.height && (selfProperty.height = shapeScale * allRect.height);
    } else {
    ...
    if (child.g) {
      ...
      if (child.path) {
        ...
        circleAnchors.forEach((item) => {
          if (item.x <= 0.01 || item.x >= 0.99 || item.y <= 0.01 || item.y >= 0.99) {
...
function getTranslate(transform) {
  ...
  let offsetY = 0;
  if (transform) {
    let matchArr = transform.replace("translate(", "").replace(")", "").split(",");
...
function transformNormalShape(childProperty, parentProperty, parentId) {
  ...
  if (finalProperty.fill === "none") {
  } else if ((_a = finalProperty.fill) == null ? void 0 : _a.includes("url")) {
    ...
    background = gradientColor == null ? void 0 : gradientColor.color;
  } else {
  ...
  return {
    ...
    lineWidth: finalProperty["stroke-width"] ? parseFloat(finalProperty["stroke-width"]) * shapeScale : finalProperty.stroke ? 1 : 0,
    ...
    fontSize: finalProperty["font-size"] ? parseFloat(finalProperty["font-size"]) * shapeScale : 16,
...
function transformText(childProperty, textContent, pen) {
  var _a;
  const text = (_a = textContent[0]) == null ? void 0 : _a[contentProp];
  ...
  return {
    ...
    textAlign: "left",
    text,
    x: (parseFloat(childProperty.x) + (offsetX || 0) - allRect.x + pen.x) / allRect.width,
    y: (parseFloat(childProperty.y) + (offsetY || 0) - allRect.y + pen.y - height) / allRect.height,
...
function styleToJson(style2) {
  if (!style2) {
    return {};
  }
  const styleArr = style2.split(";");
  const json = {};
  ...
  defs.forEach((def) => {
    if (def.linearGradient) {
      ...
      if (def.linearGradient.length > 0) {
        linearGradientItem.color = def.linearGradient[0][selfName]["stop-color"];
      }
...
function getClassStyle(className) {
  ...
  for (const key in style) {
    if (Object.prototype.hasOwnProperty.call(style, key)) {
      ...
      if (key === "." + className || key.includes("." + className + ",") || key.includes(",." + className)) {
...
function dealUnit(selfProperty) {
  var _a;
  if ((_a = String(selfProperty.width)) == null ? void 0 : _a.endsWith("in")) {
    const width2 = parseFloat(selfProperty.width) * 96;
    const height2 = parseFloat(selfProperty.height) * 96;
...
//# sourceMappingURL=@meta2d_svg.js.map
